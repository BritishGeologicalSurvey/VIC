       ...
       location_struct   *location)
{
    ...
    double                     Kaq; // hydraulic conductivity from AMBHAS (m/day)!!
    double		       z; //water table depth from AMBHAS DEM-h (m)
    ...
    double                     depth_sum[MAX_LAYERS]; //in m
    ...
    /** Set Kaq and z to the aquifer hydraulic conductivity and water table depth from AMBHAS **/
    Kaq = location->Kaq;
    z = location->z;
    ...
    for (fidx = 0; fidx < (int)options.Nfrost; fidx++) {
        ...
        **************************************************/
		...
		depth_sum[lindex]=depth[lindex];
            ...
            if(lindex>0){
		depth_sum[lindex]=depth[lindex]+depth[lindex-1];
            }

            //set bubbling pressure
	...
	zbot=depth_sum[options.Nlayer-1];	
        if(z>depth_sum[lindex]){
		water_table_layer=options.Nlayer+1;//initialise the water table below the soil layers
        }
        /********************************************************************************************
	...
	if(z<=zbot){
		lindex=water_table_layer;
                ...
                /** Calculate matric potential for layer that is above the water table
                psi_m = psi_e(moist/max_moist)^(-b)  Campbell 1985 eq 5.9**/

                psi_m=bubble[lindex] * 100.0* pow((liq[lindex]/soil_con->max_moist[lindex]),
				(3.0- soil_con->expt[lindex])/2.0);

                /** Calculate saturated matric potential for layer that is in the water table
                psi_m = psi_e(moist/max_moist)^(-b)  Campbell 1985 eq 5.9**/

                psi_sat=bubble[lindex];

                //eq 9 from Niu et al 2007 divided by runoff_steps_per_dt=runoff_steps_per_day/model_steps_per_day
                Q12[lindex]=-Ksat[lindex] / runoff_steps_per_dt * ((psi_sat-z)-
                (psi_m-depth_sum[lindex])/(z-depth_sum[lindex]));
	...
	if(water_table_layer>options.Nlayer){
		lindex=options.Nlayer - 1;
		/** Calculate matric potential psi_m = psi_e(moist/max_moist)^(-b) [m] Campbell 1985 eq 5.9**/
		//bubble[lindex] is in cm, so the equation is multiplied by 100
		psi_m=bubble[lindex] *100.0* pow((liq[lindex]/soil_con->max_moist[lindex]),
			(3.0- soil_con->expt[lindex])/2.0);
		/** Calculate recharge **/

		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//!!!!!!!Ksat needs to be changed to K from AMBHAS--is this correct now?? Kaq is in m/day and the equation is 				divided by  runoff_steps_per_dt=runoff_steps_per_day/model_steps_per_day
		dt_recharge=-Kaq / runoff_steps_per_dt * ((-z-(psi_m-depth_sum[lindex]))/
			(z-depth_sum[lindex]));
	...
	if(water_table_layer<options.Nlayer-1){
		lindex=water_table_layer-1;
		...
		/*psi_sat=bubble[water_table_layer];

		//eq 9 from Niu et al 2007 divided by runoff_steps_per_dt=runoff_steps_per_day/model_steps_per_day
		dt_recharge=-Ksat[lindex] / runoff_steps_per_dt * ((psi_sat-z)-
			(psi_m-depth_sum[lindex])/(z-depth_sum[lindex]));*/
